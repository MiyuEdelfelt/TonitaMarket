...
const ListadoEliminarPublicaciones = () => {
  ...
  useEffect(() => {
    const fetchMisPublicaciones = async () => {
      ...
      try {
        const res = await axios.get('https://bk-tonita.onrender.com/api/publications/mine', {
          headers: {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto relative">
      <h1 className="text-2xl font-bold text-green-800 mb-4">Mis Publicaciones</h1>
        ...
        <ul className="space-y-4">
          {publicaciones.map((pub) => (
            <li key={pub.id_publication} className="flex items-center justify-between border p-4 rounded shadow">
              <div>
                <h3 className="text-lg font-semibold">{pub.title_publication}</h3>
...
export default ListadoEliminarPublicaciones;
